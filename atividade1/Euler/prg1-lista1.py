...
def soma_dois_inteiros(numero_inteiro1,numero_inteiro2):
    ...
    return numero_inteiro1+numero_inteiro2
    #return soma
...
def preco_aluguel_carro(dias,km):
    ...
    return float('%.2f' % (dias * 60 + float('%.2f' % (km * 0.15))))
#    return preco
...
def dias_perdidos_por_fumar(cigarros,anos):
    ...
    return float('%.2f' %  (float('%.2f' % (float('%.2f' % (((cigarros * 365) * anos) * 10 ))/ 60)) / 24 ))
#    return dias
